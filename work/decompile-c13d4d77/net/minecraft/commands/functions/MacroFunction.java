...
public class MacroFunction<T extends ExecutionCommandSource<T>> implements CommandFunction<T> {
    ...
    public InstantiatedFunction<T> instantiate(@Nullable NBTTagCompound nbttagcompound, CommandDispatcher<T> commanddispatcher, T t0) throws FunctionInstantiationException {
        if (nbttagcompound == null) {
            ...
            if (instantiatedfunction != null) {
                ...
                InstantiatedFunction<T> instantiatedfunction1 = this.substituteAndParse(this.parameters, list, commanddispatcher, t0);
    ...
    private InstantiatedFunction<T> substituteAndParse(List<String> list, List<String> list1, CommandDispatcher<T> commanddispatcher, T t0) throws FunctionInstantiationException {
        ...
        while (iterator.hasNext()) {
            ...
            list2.add(macrofunction_a.instantiate(list3, commanddispatcher, t0, this.id));
        }
    ...
    interface a<T> {
        ...
        UnboundEntryAction<T> instantiate(List<String> list, CommandDispatcher<T> commanddispatcher, T t0, MinecraftKey minecraftkey) throws FunctionInstantiationException;
    }
    ...
    static class b<T extends ExecutionCommandSource<T>> implements MacroFunction.a<T> {
        ...
        public UnboundEntryAction<T> instantiate(List<String> list, CommandDispatcher<T> commanddispatcher, T t0, MinecraftKey minecraftkey) throws FunctionInstantiationException {
            String s = this.template.substitute(list);

            try {
                return CommandFunction.parseCommand(commanddispatcher, t0, new StringReader(s));
    ...
    static class c<T> implements MacroFunction.a<T> {
        ...
        @Override
        public UnboundEntryAction<T> instantiate(List<String> list, CommandDispatcher<T> commanddispatcher, T t0, MinecraftKey minecraftkey) {
