...
public interface CommandFunction<T> {
    ...
    InstantiatedFunction<T> instantiate(@Nullable NBTTagCompound nbttagcompound, CommandDispatcher<T> commanddispatcher, T t0) throws FunctionInstantiationException;
    ...
    static <T extends ExecutionCommandSource<T>> CommandFunction<T> fromLines(MinecraftKey minecraftkey, CommandDispatcher<T> commanddispatcher, T t0, List<String> list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (stringreader.canRead() && stringreader.peek() != '#') {
                ...
                if (stringreader.peek() == '$') {
                    functionbuilder.addMacro(s2.substring(1), j);
                } else {
                    try {
