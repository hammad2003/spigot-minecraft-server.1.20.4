...
public class EntityDrowned extends EntityZombie implements IRangedEntity {
    ...
    public static boolean checkDrownedSpawnRules(EntityTypes<EntityDrowned> entitytypes, WorldAccess worldaccess, EnumMobSpawn enummobspawn, BlockPosition blockposition, RandomSource randomsource) {
        if (!worldaccess.getFluidState(blockposition.below()).is(TagsFluid.WATER) && !EnumMobSpawn.isSpawner(enummobspawn)) {
            return false;
        } else {
            Holder<BiomeBase> holder = worldaccess.getBiome(blockposition);
            boolean flag = worldaccess.getDifficulty() != EnumDifficulty.PEACEFUL && (EnumMobSpawn.ignoresLightRequirements(enummobspawn) || isDarkEnoughToSpawn(worldaccess, blockposition, randomsource)) && (EnumMobSpawn.isSpawner(enummobspawn) || worldaccess.getFluidState(blockposition).is(TagsFluid.WATER));

            return flag && EnumMobSpawn.isSpawner(enummobspawn) ? true : (holder.is(BiomeTags.MORE_FREQUENT_DROWNED_SPAWNS) ? randomsource.nextInt(15) == 0 && flag : randomsource.nextInt(40) == 0 && isDeepEnoughToSpawn(worldaccess, blockposition) && flag);
        }
    }
