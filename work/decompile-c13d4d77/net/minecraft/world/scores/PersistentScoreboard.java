...
public class PersistentScoreboard extends PersistentBase {
    ...
    private void loadObjectives(NBTTagList nbttaglist) {
        for (int i = 0; i < nbttaglist.size(); ++i) {
            ...
            NumberFormat numberformat = (NumberFormat) NumberFormatTypes.CODEC.parse(DynamicOpsNBT.INSTANCE, nbttagcompound.get("format")).result().orElse((Object) null);
    ...
    private NBTTagList saveObjectives() {
        ...
        for (Iterator iterator = collection.iterator(); iterator.hasNext(); nbttaglist.add(nbttagcompound)) {
            ...
            if (numberformat != null) {
                NumberFormatTypes.CODEC.encodeStart(DynamicOpsNBT.INSTANCE, numberformat).result().ifPresent((nbtbase) -> {
