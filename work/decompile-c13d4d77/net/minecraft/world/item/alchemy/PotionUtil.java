...
public class PotionUtil {
    ...
    public static void getCustomEffects(@Nullable NBTTagCompound nbttagcompound, List<MobEffect> list) {
        if (nbttagcompound != null && nbttagcompound.contains("custom_potion_effects", 9)) {
            NBTTagList nbttaglist = nbttagcompound.getList("custom_potion_effects", 10);
    ...
    public static ItemStack setCustomEffects(ItemStack itemstack, Collection<MobEffect> collection) {
        if (collection.isEmpty()) {
            ...
            NBTTagList nbttaglist = nbttagcompound.getList("custom_potion_effects", 9);
            Iterator iterator = collection.iterator();
            ...
            nbttagcompound.put("custom_potion_effects", nbttaglist);
            return itemstack;
        }
    }

    public static void addPotionTooltip(ItemStack itemstack, List<IChatBaseComponent> list, float f, float f1) {
        addPotionTooltip(getMobEffects(itemstack), list, f, f1);
    }

    public static void addPotionTooltip(List<MobEffect> list, List<IChatBaseComponent> list1, float f, float f1) {
        ...
        if (list.isEmpty()) {
            ...
            for (iterator = list.iterator(); iterator.hasNext(); list1.add(ichatmutablecomponent.withStyle(mobeffectlist.getCategory().getTooltipFormatting()))) {
                ...
                Map<AttributeBase, AttributeModifierTemplate> map = mobeffectlist.getAttributeModifiers();

                if (!map.isEmpty()) {
                    Iterator iterator1 = map.entrySet().iterator();

                    while (iterator1.hasNext()) {
                        Entry<AttributeBase, AttributeModifierTemplate> entry = (Entry) iterator1.next();

                        list2.add(new Pair((AttributeBase) entry.getKey(), ((AttributeModifierTemplate) entry.getValue()).create(mobeffect.getAmplifier())));
                    }
                }
        ...
        if (!list2.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (d0 > 0.0D) {
                    list1.add(IChatBaseComponent.translatable("attribute.modifier.plus." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), IChatBaseComponent.translatable(((AttributeBase) pair.getFirst()).getDescriptionId())).withStyle(EnumChatFormat.BLUE));
                } else if (d0 < 0.0D) {
                    d1 *= -1.0D;
                    list1.add(IChatBaseComponent.translatable("attribute.modifier.take." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), IChatBaseComponent.translatable(((AttributeBase) pair.getFirst()).getDescriptionId())).withStyle(EnumChatFormat.RED));
                }
            }
        }
