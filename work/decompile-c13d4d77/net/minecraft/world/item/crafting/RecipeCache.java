...
public class RecipeCache {
    ...
    private Optional<RecipeCrafting> compute(InventoryCrafting inventorycrafting, World world) {
        ...
        this.insert(inventorycrafting.getItems(), (RecipeCrafting) optional.map(RecipeHolder::value).orElse((Object) null));
    ...
    private void insert(List<ItemStack> list, @Nullable RecipeCrafting recipecrafting) {
        ...
        this.entries[0] = new RecipeCache.a(nonnulllist, recipecrafting);
    }

    private static record a(NonNullList<ItemStack> key, @Nullable RecipeCrafting value) {
        ...
        public boolean matches(List<ItemStack> list) {
            if (this.key.size() != list.size()) {
                ...
                for (int i = 0; i < this.key.size(); ++i) {
                    if (!ItemStack.isSameItemSameTags((ItemStack) this.key.get(i), (ItemStack) list.get(i))) {
                        return false;
                    }
                }
