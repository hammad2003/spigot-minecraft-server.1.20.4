...
public class CrafterBlock extends BlockTileEntity {
    ...
    protected void dispenseFrom(IBlockData iblockdata, WorldServer worldserver, BlockPosition blockposition) {
        ...
        if (tileentity instanceof CrafterBlockEntity) {
            CrafterBlockEntity crafterblockentity = (CrafterBlockEntity) tileentity;
            Optional optional = getPotentialResults(worldserver, crafterblockentity);
            ...
            if (optional.isEmpty()) {
                ...
                ItemStack itemstack = recipecrafting.assemble(crafterblockentity, worldserver.registryAccess());

                itemstack.onCraftedBySystem(worldserver);
                this.dispenseItem(worldserver, blockposition, crafterblockentity, itemstack, iblockdata);
                recipecrafting.getRemainingItems(crafterblockentity).forEach((itemstack1) -> {
                    this.dispenseItem(worldserver, blockposition, crafterblockentity, itemstack1, iblockdata);
                });
                crafterblockentity.getItems().forEach((itemstack1) -> {
    ...
    public static Optional<RecipeCrafting> getPotentialResults(World world, InventoryCrafting inventorycrafting) {
        return CrafterBlock.RECIPE_CACHE.get(world, inventorycrafting);
    }

    private void dispenseItem(World world, BlockPosition blockposition, CrafterBlockEntity crafterblockentity, ItemStack itemstack, IBlockData iblockdata) {
        EnumDirection enumdirection = ((BlockPropertyJigsawOrientation) iblockdata.getValue(CrafterBlock.ORIENTATION)).front();
        IInventory iinventory = TileEntityHopper.getContainerAt(world, blockposition.relative(enumdirection));
        ItemStack itemstack1 = itemstack.copy();

        if (iinventory != null && (iinventory instanceof CrafterBlockEntity || itemstack.getCount() > iinventory.getMaxStackSize())) {
        ...
        if (!itemstack1.isEmpty()) {
            Vec3D vec3d = Vec3D.atCenterOf(blockposition).relative(enumdirection, 0.7D);

            DispenseBehaviorItem.spawnItem(world, itemstack1, 6, enumdirection, vec3d);
            world.levelEvent(1049, blockposition, 0);
            world.levelEvent(2010, blockposition, enumdirection.get3DDataValue());
        }
