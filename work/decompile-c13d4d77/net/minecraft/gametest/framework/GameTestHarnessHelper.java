...
public class GameTestHarnessHelper {
    ...
    public <E extends Entity> E spawn(EntityTypes<E> entitytypes, Vec3D vec3d) {
        ...
        if (e0 == null) {
            throw new NullPointerException("Failed to create entity " + entitytypes.builtInRegistryHolder().key().location());
        } else {
    ...
    public void useBlock(BlockPosition blockposition) {
        this.useBlock(blockposition, this.makeMockPlayer());
    }
    ...
    public void useBlock(BlockPosition blockposition, EntityHuman entityhuman, MovingObjectPositionBlock movingobjectpositionblock) {
        BlockPosition blockposition1 = this.absolutePos(blockposition);
        IBlockData iblockdata = this.getLevel().getBlockState(blockposition1);
        EnumInteractionResult enuminteractionresult = iblockdata.use(this.getLevel(), entityhuman, EnumHand.MAIN_HAND, movingobjectpositionblock);
        ...
        if (!enuminteractionresult.consumesAction()) {
            ...
            entityhuman.getItemInHand(EnumHand.MAIN_HAND).useOn(itemactioncontext);
        }
    ...
    public EntityPlayer makeMockServerPlayerInLevel() {
        ...
        embeddedchannel.attr(NetworkManager.ATTRIBUTE_SERVERBOUND_PROTOCOL).set(EnumProtocol.PLAY.codec(EnumProtocolDirection.SERVERBOUND));
    ...
    public <T extends Comparable<T>> void assertBlockProperty(BlockPosition blockposition, IBlockState<T> iblockstate, T t0) {
        ...
        if (!flag || !iblockdata.getValue(iblockstate).equals(t0)) {
            String s = flag ? "was " + iblockdata.getValue(iblockstate) : "property " + iblockstate.getName() + " is missing";
    ...
    public <E extends EntityLiving> void assertEntityIsHolding(BlockPosition blockposition, EntityTypes<E> entitytypes, Item item) {
        ...
        if (list.isEmpty()) {
            throw new GameTestHarnessAssertionPosition("Expected entity of type: " + entitytypes, blockposition1, blockposition, this.getTick());
        } else {
            Iterator iterator = list.iterator();
            ...
            do {
                if (!iterator.hasNext()) {
                    throw new GameTestHarnessAssertionPosition("Entity should be holding: " + item, blockposition1, blockposition, this.getTick());
                }
    ...
    public <E extends Entity & InventoryCarrier> void assertEntityInventoryContains(BlockPosition blockposition, EntityTypes<E> entitytypes, Item item) {
        ...
        if (list.isEmpty()) {
            ...
            do {
                if (!iterator.hasNext()) {
                    throw new GameTestHarnessAssertionPosition("Entity inventory should contain: " + item, blockposition1, blockposition, this.getTick());
                }
    ...
    public void assertContainerContains(BlockPosition blockposition, Item item) {
        ...
        if (!(tileentity instanceof TileEntityContainer)) {
            throw new GameTestHarnessAssertion("Expected a container at " + blockposition + ", found " + BuiltInRegistries.BLOCK_ENTITY_TYPE.getKey(tileentity.getType()));
    ...
    public void assertLivingEntityHasMobEffect(EntityLiving entityliving, MobEffectList mobeffectlist, int i) {
        MobEffect mobeffect = entityliving.getEffect(mobeffectlist);

        if (mobeffect == null || mobeffect.getAmplifier() != i) {
            int j = i + 1;

            throw new GameTestHarnessAssertion("Entity " + entityliving + " failed has " + mobeffectlist.getDescriptionId() + " x " + j + " test");
        }
    }
