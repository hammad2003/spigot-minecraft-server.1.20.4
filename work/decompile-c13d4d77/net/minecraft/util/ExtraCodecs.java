...
public class ExtraCodecs {
    ...
    public static final Codec<JsonElement> FLAT_JSON = Codec.STRING.flatXmap((s) -> {
        try {
            return DataResult.success(JsonParser.parseString(s));
        ...
        try {
            return DataResult.success(ChatDeserializer.toStableString(jsonelement));
    ...
    public static <E> MapCodec<E> orCompressed(final MapCodec<E> mapcodec, final MapCodec<E> mapcodec1) {
        return new MapCodec<E>() {
            ...
            public String toString() {
                return mapcodec + " orCompressed " + mapcodec1;
            }
        };
    }
    ...
    public static <T> Codec<T> recursive(String s, Function<Codec<T>, Codec<T>> function) {
        return new ExtraCodecs.c<>(s, function);
    }

    public static <A> Codec<A> lazyInitializedCodec(Supplier<Codec<A>> supplier) {
        return new ExtraCodecs.c<>(supplier.toString(), (codec) -> {
    ...
    private static class c<T> implements Codec<T> {
        ...
        c(String s, Function<Codec<T>, Codec<T>> function) {
            this.name = s;
