...
public class ResetChunksCommand {
    ...
    private static int resetChunks(CommandListenerWrapper commandlistenerwrapper, int i, boolean flag) {
        ...
        while (unmodifiableiterator.hasNext()) {
                ...
                for (int l2 = chunkcoordintpair.x - i; l2 <= chunkcoordintpair.x + i; ++l2) {
                    ...
                    if (chunk1 != null && (!flag || !chunk1.isOldNoiseGeneration())) {
                        ...
                        Function function = (unit) -> {
                            Objects.requireNonNull(threadedmailbox);
                            return chunkstatus.generate(threadedmailbox::tell, worldserver, chunkproviderserver.getGenerator(), worldserver.getStructureManager(), chunkproviderserver.getLightEngine(), (ichunkaccess1) -> {
                                throw new UnsupportedOperationException("Not creating full chunks here");
                            }, list).thenApply((either) -> {
                                if (chunkstatus == ChunkStatus.NOISE) {
                                    either.left().ifPresent((ichunkaccess1) -> {
            ...
            ResetChunksCommand.LOGGER.debug(chunkstatus + " took " + (System.currentTimeMillis() - j2) + " ms");
        }
