...
public class RaidCommand {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("raid").requires((commandlistenerwrapper) -> {
        ...
        }))).then(net.minecraft.commands.CommandDispatcher.literal("sound").then(net.minecraft.commands.CommandDispatcher.argument("type", ArgumentChatComponent.textComponent()).executes((commandcontext) -> {
            ...
            return setBadOmenLevel((CommandListenerWrapper) commandcontext.getSource(), IntegerArgumentType.getInteger(commandcontext, "level"));
    ...
    private static int setBadOmenLevel(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        Raid raid = getRaid(commandlistenerwrapper.getPlayerOrException());
        ...
        if (raid != null) {
            ...
            if (i > j) {
                commandlistenerwrapper.sendFailure(IChatBaseComponent.literal("Sorry, the max bad omen level you can set is " + j));
            } else {
                int k = raid.getBadOmenLevel();

                raid.setBadOmenLevel(i);
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.literal("Changed village's bad omen level from " + k + " to " + i);
                }, false);
            }
        } else {
    ...
    private static int spawnLeader(CommandListenerWrapper commandlistenerwrapper) {
        ...
        if (entityraider == null) {
            ...
            entityraider.setItemSlot(EnumItemSlot.HEAD, Raid.getLeaderBannerInstance());
            ...
            entityraider.finalizeSpawn(commandlistenerwrapper.getLevel(), commandlistenerwrapper.getLevel().getCurrentDifficultyAt(BlockPosition.containing(commandlistenerwrapper.getPosition())), EnumMobSpawn.COMMAND, (GroupDataEntity) null, (NBTTagCompound) null);
    ...
    private static int start(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        ...
        if (entityplayer.serverLevel().isRaided(blockposition)) {
            ...
            Raid raid = persistentraid.createOrExtendRaid(entityplayer);
