...
public record ResourcePackInfo(IChatBaseComponent description, int packFormat, Optional<InclusiveRange<Integer>> supportedFormats) {
    ...
    public static final Codec<ResourcePackInfo> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(ComponentSerialization.CODEC.fieldOf("description").forGetter(ResourcePackInfo::description), Codec.INT.fieldOf("pack_format").forGetter(ResourcePackInfo::packFormat), InclusiveRange.codec(Codec.INT).optionalFieldOf("supported_formats").forGetter(ResourcePackInfo::supportedFormats)).apply(instance, ResourcePackInfo::new);
    });
